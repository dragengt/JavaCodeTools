...
namespace JavaTools.Util
{
	...
	{{
		this.{4} = {3};
                ...
                //不带注释的情况下的匹配字段内容：（可能在//代码段中）
                MatchCollection matchFiled = Regex.Matches(proccedingContent, @"([\w<,>]+) (\w)(\w+);", RegexOptions.Multiline);

                //检查字段如果是带初始化值的，则抛错：
                MatchCollection privateFiledsWithInit = Regex.Matches(fileContent, @"private\s*([\w<,>]+) (\w)(\w+)\s*=.*", RegexOptions.Multiline);
                if (privateFiledsWithInit.Count > 0)
                {
                    unableList.Add(currFile+"\t存在非规范内容：在字段中初始化值，而没有将其放在构造方法中.");
                    return false;
                }
