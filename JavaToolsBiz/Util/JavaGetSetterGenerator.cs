...
namespace JavaToolsBiz.Util
{
    public class JavaGetSetterGenerator
    {
        ...
        const string C_RegexMatchFileWithComment = @"/\*\*[\n\s\*]+(.*)[*\s]*\s+\*/\s+private ([\w<, >]+) (\w)(\w+)\s*;";
            ...
            processResultList = CommonUtil.ForeachJavaFiles(folderPath, errorList, (currFile, fileContent) =>
             {
                 ...
                 bool isSucc = ProcessFileContent(currFile, fileContent, false, errorList, out codeBuilder);
        ...
        /// <returns></returns>
        public static string ConvertCodeWithGetSetter(string text, bool genSwaggerComment)
        {
        ...
        /// <returns></returns>
        private static bool ProcessFileContent(string currFile, string fileContent, bool genSwaggerComment, List<string> errorMsg, out StringBuilder fileContentStringBuilder)
        {
            ...
            MatchCollection matchFiledWithComment = Regex.Matches(proccedingContent, C_RegexMatchFileWithComment, RegexOptions.Multiline);
            ...
            if (genSwaggerComment)
            {
                newFileContent = GenSwaggerComment(newFileContent);
            }
...
",
        ...
        private static string GenSwaggerComment(string fileContent)
        {
            // 匹配注释部分，将之替换为APIModelProperty注解
            //TODO: 目前替换问题：带注解的字段无法做处理
            return Regex.Replace(fileContent, C_RegexMatchFileWithComment, (matched) =>
            {
